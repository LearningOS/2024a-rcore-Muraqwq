...
impl ProcessControlBlock {
    ...
    pub fn exec(self: &Arc<Self>, elf_data: &[u8], args: Vec<String>) {
        ...
        process_inner.available = (Vec::new(), Vec::new());
    ...
    pub fn deadlock_detect(&self) -> bool {
        ...
        loop {
            ...
            if !work.0.is_empty() {
                for mutex in 0..work.0.len() {
                    work.0[mutex] += inner.allocation[thread].0[mutex];
                }
            }

            if !work.1.is_empty() {
                for sem in 0..work.1.len() {
                    work.1[sem] += inner.allocation[thread].1[sem];
                }
            }
        }
    }
    ///
    pub fn change_deadlock_enable(&self, enable: usize) {
        self.inner_exclusive_access().dead_lock_detect_enable = enable;
    }
    ///
    pub fn deadlock_enable(&self) -> bool {
        self.inner_exclusive_access().dead_lock_detect_enable == 1
    }
}
